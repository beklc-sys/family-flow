...
import { addFavoriteToShoppingList, createFavorite, deleteFavorite, ensureRecurringFavoritesForDate, loadFavorites, updateFavorite } from "./favorites.js";
...
function bindEvents() {
  ...
  elements.favoriteForm.addEventListener("submit", async (event) => { event.preventDefault(); const id = elements.favoriteId.value; const changes = { item_text: elements.favoriteText.value.trim(), category: elements.favoriteCategory.value.trim() || "Sonstiges", repeat_weekday: elements.favoriteRepeat.value }; await runAction(async () => { if (id) await updateFavorite(id, changes); else await createFavorite(changes.item_text, changes.category, changes.repeat_weekday); resetFavoriteForm(); showToast("Favorit gespeichert"); }); });
...
async function enterFamily(family) {
  ...
  if (navigator.onLine) { const synchronized = await flushPendingOperations(); if (synchronized) await loadShoppingItems(family.id); await loadFavorites(family.id); await ensureRecurringFavoritesForDate(getState().selectedDate); }
